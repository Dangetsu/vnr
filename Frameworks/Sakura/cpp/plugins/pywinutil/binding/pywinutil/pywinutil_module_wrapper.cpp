...
PyTypeObject** SbkpywinutilTypes;
// Current module's converter array.
SbkConverter** SbkpywinutilTypeConverters;
// Global functions ------------------------------------------------------------

static PyMethodDef pywinutil_methods[] = {
    {0} // Sentinel
};
...
#endif
SBK_MODULE_INIT_FUNCTION_BEGIN(pywinutil)
    // Create an array of wrapper types for the current module.
    static PyTypeObject* cppApi[SBK_pywinutil_IDX_COUNT];
    SbkpywinutilTypes = cppApi;

    // Create an array of primitive type converters for the current module.
    static SbkConverter* sbkConverters[SBK_pywinutil_CONVERTERS_IDX_COUNT];
    SbkpywinutilTypeConverters = sbkConverters;

#ifdef IS_PY3K
    PyObject* module = Shiboken::Module::create("pywinutil", &moduledef);
#else
    PyObject* module = Shiboken::Module::create("pywinutil", pywinutil_methods);
#endif
    ...
    SbkpywinutilTypeConverters[SBK_STD_WSTRING_IDX] = Shiboken::Conversions::createConverter(&PyUnicode_Type, std_wstring_CppToPython_std_wstring);
    Shiboken::Conversions::registerConverterName(SbkpywinutilTypeConverters[SBK_STD_WSTRING_IDX], "std::wstring");
    // Add user defined implicit conversions to type converter.
    Shiboken::Conversions::addPythonToCppValueConversion(SbkpywinutilTypeConverters[SBK_STD_WSTRING_IDX],
    ...
    Shiboken::Module::registerTypes(module, SbkpywinutilTypes);
    Shiboken::Module::registerTypeConverters(module, SbkpywinutilTypeConverters);

    if (PyErr_Occurred()) {
        PyErr_Print();
        Py_FatalError("can't initialize module pywinutil");
    }
